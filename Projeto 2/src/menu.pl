...
play_menu :-
    ...
    ;   integer(BoardSizeOption), board_size(BoardSizeOption, Size) -> 
        ...
        ;   integer(GameTypeOption), game_type(GameTypeOption, GameType) -> 
            (   GameType == pvc -> choose_computer_difficulty(Size, GameType)
            ;   nl, write('Starting game...'), nl, start_game(Size, GameType)
            )
        ;   write('Invalid game type. Returning to play menu.'), nl, play_menu
        )
    ;   write('Invalid board size. Returning to play menu.'), nl, play_menu
    ).

% Select computer difficulty
choose_computer_difficulty(Size, GameType) :-
    nl,
    ...
    ;   integer(DifficultyOption), difficulty(DifficultyOption, Difficulty) -> 
        nl, write('Starting game with difficulty: '), write(Difficulty), nl,
        start_game(Size, GameType, Difficulty)
    ;   write('Invalid choice. Returning to play menu.'), nl, play_menu
    ).
